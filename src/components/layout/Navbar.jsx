...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${isDiscover ? 'bg-white border-b border-gray-100' : 'glass border-b border-border/50'}`}>
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${isDiscover ? 'w-full max-w-[1600px]' : 'max-w-7xl'}`}>
        <div className="flex items-center justify-between h-16">
          ...
          <Link to={user ? '/dashboard' : '/'} className="flex items-center gap-2 group">
            ...
            <span className={`font-heading font-bold text-xl tracking-tight ${isDiscover ? 'text-[#0f172a]' : 'gradient-text'}`}>
              StudyMatch
            </span>
          </Link>
          ...
          {user && isDiscover && (
            <div className="hidden md:flex items-center gap-6 absolute left-1/2 -translate-x-1/2">
              ...
              <Link to="/matches" className="text-sm font-medium text-gray-700 hover:text-blue-600 transition-colors">
                Matches
              </Link>
              <Link to="/chat" className="text-sm font-medium text-blue-500 hover:text-blue-600 transition-colors">
                Chat
              </Link>
            ...
            {user ? (
              isDiscover ? (
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#fcd34d] overflow-hidden flex items-center justify-center cursor-pointer border-2 border-white shadow-sm">
                    <div className="w-full h-full bg-orange-100 flex items-center justify-center">
                      <User className="w-5 h-5 text-orange-400 fill-orange-400" />
                    </div>
                  </div>
                </div>
              ) : (
